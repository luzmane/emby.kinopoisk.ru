...
public class CreateTop250MovieCollectionsTaskTest : BaseTest
{
    ...
    [Fact]
    public async void CreateTop250MovieCollectionsTask_Execute_CollectionNotExists_OneLib()
    {
        ...
        _ = _xmlSerializer
            .Setup(m => m.DeserializeFromFile(typeof(LibraryOptions), "CreateTop250MovieCollectionsTask_Execute_CollectionNotExists_OneLib/options.xml"))
        ...
        _ = _libraryManager
            .Setup(m => m.QueryItems(It.Is<InternalItemsQuery>(query =>
                !query.Recursive
                && query.Name == _pluginConfiguration.GetCurrentTop250MovieCollectionName()
                && query.IncludeItemTypes.Length == 1
                && "BoxSet".Equals(query.IncludeItemTypes[0], StringComparison.Ordinal))))
            .Returns(new QueryResult<BaseItem>()
            {
        ...
        _ = _libraryManager
            ...
            {
                Name = "Collections",
                Path = "CreateTop250MovieCollectionsTask_Execute_CollectionNotExists_OneLib"
            });

        _ = _applicationPaths
            .SetupGet(m => m.PluginConfigurationsPath)
            .Returns("CreateTop250MovieCollectionsTask_Execute_CollectionNotExists_OneLib");
        ...
        _logManager.Verify(lm => lm.GetLogger(It.IsAny<string>()), Times.Exactly(4));
        _applicationPaths.VerifyGet(ap => ap.PluginConfigurationsPath, Times.Once());
        _xmlSerializer.Verify(xs => xs.DeserializeFromFile(typeof(PluginConfiguration), "CreateTop250MovieCollectionsTask_Execute_CollectionNotExists_OneLib/EmbyKinopoiskRu.xml"), Times.Once());
        _xmlSerializer.Verify(xs => xs.DeserializeFromFile(typeof(LibraryOptions), "/emby/video_library_onelib/options.xml"), Times.Once());
        _xmlSerializer.Verify(xs => xs.DeserializeFromFile(typeof(LibraryOptions), "CreateTop250MovieCollectionsTask_Execute_CollectionNotExists_OneLib/options.xml"), Times.Once());
        _libraryManager.Verify(lm => lm.QueryItems(It.IsAny<InternalItemsQuery>()), Times.Exactly(3));
        ...
        Logger.Info($"Finish '{nameof(CreateTop250MovieCollectionsTask_Execute_CollectionNotExists_OneLib)}'");
    }
    ...
    public async void CreateTop250MovieCollectionsTask_Execute_CollectionNotExists()
    {
        ...
        _ = _libraryManager
            .Setup(m => m.QueryItems(It.Is<InternalItemsQuery>(query =>
                !query.Recursive
                && query.Name == $"{_pluginConfiguration.GetCurrentTop250MovieCollectionName()} (My Movies)"
                && query.IncludeItemTypes.Length == 1
                && "BoxSet".Equals(query.IncludeItemTypes[0], StringComparison.Ordinal))))
            .Returns(new QueryResult<BaseItem>()
            {
